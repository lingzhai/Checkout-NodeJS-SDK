...
class PayPalHttpClient extends paypalhttp.HttpClient {
  ...
  execute(request) {
    console.log('----paypal client execute start');
    console.log('---paypal execute request:',request);
    const result = super.execute(request).catch((err) => {
      console.log('----paypal client execute err:',err);
      if (err.statusCode === 401) {
        return this._retryRequest(request);
      }
      return Promise.reject(err);
    });
    console.log('---paypal execute result:',result);
    return result;
  }
